...
var kDefaultScale = (isMobile==0)?'page-fit':'0.5';
...
function updateDepth(nbrPages, currentPage) {
	var depthLimitPage = 10;
	if (nbrPages<depthLimitPage){
		depthLimitPage = nbrPages;
	}
	
	var currentPageSim = currentPage*depthLimitPage/nbrPages;
	
	var nbrPasTotal = 7*depthLimitPage/5;
	
	var unPas =  nbrPasTotal/depthLimitPage;
	
	var pasgauche= currentPageSim*unPas;
	var pasdroite= nbrPasTotal-pasgauche;
	
	
	var leftMinDepth = 19;
	var rightMinDepth = 19;
	
	var leftDepth = $(".front-side .depth");
	var rightDepth = $(".back-side .depth");
	
	leftDepth.css({left:leftMinDepth - pasgauche+'px'});
	rightDepth.css({right:rightMinDepth-pasdroite+'px'});
...
var PDFView = {
  ...
  load: function pdfViewLoad(pdfDocument, scale) {
    ...
    document.getElementById('numPages').textContent =
      mozL10n.get('page_of', {pageCount: pagesCount}, 'sur {{pageCount}}');
    ...
    pagesPromise.then(function(promisedPages) {
      for (var i = 1; i <= pagesCount; i++) {
        ...
        if(pageView.viewport.width<pageView.viewport.height){
        	pagesRotation = 0;
        }else{
        	pagesRotation = 90;
        }
        //pagesRotation = page.pageInfo.rotate;
      }
    ...
    // turn
    	$('#viewer').turn({acceleration: true, 
							...
							gradients: !$.isTouch,
							duration : 600,
							display: (pagesRotation == 0)?'double':'single',
							 autoCenter : true,
							when: {
...
var TextLayerBuilder = function textLayerBuilder(textLayerDiv) {
  ...
  this.appendText = function textLayerBuilderAppendText(text,
    ...
    var fontHeight = fontSize * text.vScale;
    textDiv.dataset.canvasWidth = text.canvasWidth * text.hScale;
    textDiv.dataset.fontName = fontName;

    textDiv.style.fontSize = fontHeight + 'px';
    textDiv.style.left = text.x + 'px';
    textDiv.style.top = (text.y - fontHeight) + 'px';
...
if (window.addEventListener){
	window.addEventListener('load', webViewerLoad, true);
}else{
	window.attachEvent("onload", webViewerLoad, true);
}
function webViewerLoad(evt) {
  ...
  if (window.addEventListener){
		thumbsView.addEventListener('scroll', updateThumbViewArea, true);  	
  		...
  		thumbsView.attachEvent('onscroll', updateThumbViewArea, true);
  }

  var mainContainer = document.getElementById('mainContainer');
  var outerContainer = document.getElementById('outerContainer');
  if (window.addEventListener){
  		mainContainer.addEventListener('transitionend', webTransitionend,true);
  		document.getElementById('sidebarToggle').addEventListener('click', webClickDocument, true);
  }else{
  		mainContainer.attachEvent('ontransitionend', webTransitionend,true);
  		document.getElementById('sidebarToggle').attachEvent('onclick', webClickDocument, true);
  }
  PDFView.open(file, 0);
  if (isMobile == 0){
  	$("#sidebarToggle").click();
...
if (window.addEventListener){
	window.addEventListener('scroll', webViewerScroll, true);
}else{
	window.attachEvent("onscroll", webViewerScroll, true);
}
...
if (window.addEventListener){
	...
	window.attachEvent("onresize", webViewerResize);
}
...
if(window.addEventListener){
	window.addEventListener('hashchange', webViewerHashchange, true);
	...
	window.attachEvent("onhashchange", webViewerHashchange, true);
	window.attachEvent("onchange", webViewerChange, true);
	window.attachEvent('onlocalized', localized, true);
	window.attachEvent('onscalechange', scalechange, true);
	window.attachEvent('onpagechange', pagechange, true);
...
function scalechange(evt) {
  ...
  //var pageRotation = page.pdfPage.pageInfo.rotate;
  if (w<h){
  	$('#viewer').turn('size', w*2, h);
  }else{
  	$('#viewer').turn('size', w, h);
  }
  $('.depth').css({height:h});
